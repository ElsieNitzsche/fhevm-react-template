...
function useFHEVM(options) {
  ...
  const encryptNumber2 = (0, import_react.useCallback)(
    async (value, bits = 8) => {
  ...
  const encryptBoolean2 = (0, import_react.useCallback)(async (value) => {
    if (!clientRef.current) {
      throw new Error("FHEVM client not initialized");
    }
    return clientRef.current.encryptBoolean(value);
  }, []);
  const encryptAddress2 = (0, import_react.useCallback)(async (address) => {
    if (!clientRef.current) {
      throw new Error("FHEVM client not initialized");
    }
    return clientRef.current.encryptAddress(address);
  }, []);
  const userDecrypt2 = (0, import_react.useCallback)(
  ...
  const generatePermit2 = (0, import_react.useCallback)(
...
function useFHEVMContract(options) {
  ...
  const contract = (0, import_react2.useMemo)(() => {
  ...
  const sendEncrypted = (0, import_react2.useCallback)(
...
function createFHEVMContract(address, abi, signer, fhevmClient) {
  const contract = new import_ethers2.Contract(address, abi, signer);
  return {
    contract,
    fhevmClient,
